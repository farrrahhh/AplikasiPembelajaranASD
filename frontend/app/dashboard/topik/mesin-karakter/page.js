...
function MateriContent() {
  return (
    <div className="text-[15px] text-gray-700">
      ...
      <P>
        <strong>Mesin</strong> adalah mekanisme yang terdefinisi dan mampu mengeksekusi
        aksi-aksi primitif yang terdefinisi untuknya.
      </P>
      <P>
        <strong>Mesin abstrak</strong> adalah mesin yang <em>dianggap ada</em> dan diasumsikan
        mampu melakukan mekanisme yang didefinisikan. Mesin abstrak memodelkan suatu semesta
        (<em>universe</em>) tertentu.
      </P>
...
mesin karakter`}</AsciiBox>
      ...
      <div className="my-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-[14px]">
        {[
          { name: 'Pita', desc: 'Deret karakter, diakhiri MARK berupa titik (\'.\').\nPita yang hanya berisi MARK disebut pita kosong.', color: 'bg-blue-50 border-blue-200 text-blue-900' },
          { name: 'Tombol start & adv', desc: 'start → menyiapkan pita untuk dibaca.\nadv → memajukan pita satu karakter.', color: 'bg-purple-50 border-purple-200 text-purple-900' },
          { name: 'Jendela (CC)', desc: 'Berukuran satu karakter.\nCC = Current Character: karakter yang sedang tampak.', color: 'bg-teal-50 border-teal-200 text-teal-900' },
          { name: 'Lampu EOP', desc: 'End of Pita: menyala (true) ketika CC = \'.\'\nJika EOP menyala, mesin tidak dapat dioperasikan lagi.', color: 'bg-orange-50 border-orange-200 text-orange-900' },
        ].map((item) => (
          <div key={item.name} className={`border rounded-xl px-4 py-3 ${item.color}`}>
            <p className="font-bold text-[13px] mb-1">{item.name}</p>
            <p className="text-[12px] whitespace-pre-line opacity-80">{item.desc}</p>
          </div>
        ))}
      </div>
      <P>State mesin karakter ditentukan oleh <strong>CC</strong> dan <strong>EOP</strong>.</P>
...
Ketika CC = '.', lampu EOP menyala.`}</AsciiBox>
      <NoteBox>
        EOP diwakili oleh boolean: <Mono>true</Mono> jika menyala, <Mono>false</Mono> jika tidak.
...
{ Pita dimajukan satu karakter.
  ...
  F.S.: cc adalah karakter berikutnya dari cc yang lama,
      ...
      <InfoBox>
        <p className="text-sm font-semibold text-purple-800 mb-1">Pola Pemrosesan Mesin Karakter</p>
...
{ cc = MARK → selesai }`}</pre>
      ...
      <P><strong>mesinkar.h</strong> — antarmuka:</P>
...
#endif
`}</CodeBlock>
      <P><strong>mesinkar.c</strong> — implementasi:</P>
      <CodeBlock language="c">{`
#include <stdio.h>
#include "mesinkar.h"
...
`}</CodeBlock>
      <NoteBox>
        Variabel <Mono>cc</Mono> dan <Mono>eop</Mono> bersifat <Mono>extern</Mono> — dideklarasikan
        di .h, didefinisikan di .c, dan dapat diakses oleh semua modul yang meng-include header-nya.
      </NoteBox>
      <Divider />
...
ALGORITMA
    ...
    output(ctr)`}</Pseudocode>
      ...
      <P>
        <strong>Mesin Kata</strong> adalah mesin abstrak yang bekerja memproses kata{' '}
        <em>berdasarkan mesin karakter</em>. Kata didefinisikan sebagai:
      </P>
      <InfoBox>
        <p className="text-sm text-purple-800">
          <strong>Kata</strong> = sederetan karakter suksesif pada pita yang merupakan karakter{' '}
          <strong>bukan blank</strong>
        </p>
      </InfoBox>
      <P><strong>Definisi type Kata (notasi algoritmik):</strong></P>
...
`}</CodeBlock>
      ...
      <P>Ada tiga versi model akuisisi kata, tergantung bagaimana blank dan akhir pita ditangani:</P>
      <W3Table
        ...
        rows={[
          ...
          ['Versi 3', 'cc = MARK langsung', 'Tanpa variabel tambahan, ada initAkses'],
        ]}
      />
...
ALGORITMA
    ...
    currentKata.length ← i`}</Pseudocode>
      ...
      {/* ── 5. Versi 1 ───────────────────────────────────────── */}
      <SectionHeading id="versi-1">Model Akuisisi Kata Versi 1 — <code className="font-mono text-[16px]">endKata</code></SectionHeading>
      <P>
        State tambahan: <Mono>endKata: boolean</Mono> dan <Mono>currentKata: Kata</Mono>.
        Akhir proses ditandai oleh <Mono>endKata = true</Mono>.
      </P>
...
ALGORITMA
    ...
    if (cc = MARK) then
      ...
      <P><strong>Pola penggunaan Versi 1:</strong></P>
...
`}</CodeBlock>
      ...
      {/* ── 6. Versi 2 ───────────────────────────────────────── */}
      <SectionHeading id="versi-2">Model Akuisisi Kata Versi 2 — <code className="font-mono text-[16px]">length = 0</code></SectionHeading>
      <P>
        Seperti versi 1, tetapi <strong>akhir proses ditandai oleh kata kosong</strong>{' '}
      ...
      <W3Table
        ...
        rows={[
          ...
          ['salinKata saat MARK', 'dicek sebelum salinKata', 'salinKata langsung → length = 0'],
        ]}
      />

      <P><strong>startKata versi 2:</strong></P>
...
{ salinKata langsung dipanggil; jika cc = MARK, maka loop repeat
  tidak berjalan karena kondisi until langsung terpenuhi → length = 0 }`}</Pseudocode>
      <P><strong>advKata versi 2:</strong></P>
      <Pseudocode>{`ALGORITMA
    ignoreBlank
    salinKata`}</Pseudocode>
      <P><strong>Pola penggunaan Versi 2:</strong></P>
      <Pseudocode>{`startKata
while currentKata.length ≠ 0 do
    { proses currentKata }
    advKata`}</Pseudocode>
      <NoteBox>
        Pada versi 2, <Mono>salinKata</Mono> perlu dimodifikasi: loop <Mono>repeat-until</Mono>
        diganti dengan <Mono>while</Mono> agar langsung menghasilkan <Mono>length = 0</Mono>
        ketika <Mono>cc = MARK</Mono> sejak awal.
      </NoteBox>
      <Divider />

      {/* ── 7. Versi 3 ───────────────────────────────────────── */}
      <SectionHeading id="versi-3">Model Akuisisi Kata Versi 3 — <code className="font-mono text-[16px]">cc = MARK</code></SectionHeading>
      <P>
        Versi 3 menggunakan <Mono>cc</Mono> langsung sebagai kondisi loop — tanpa variabel
        tambahan. Model ini membutuhkan prosedur <strong><Mono>initAkses</Mono></strong> yang
        memposisikan <Mono>cc</Mono> pada karakter pertama kata pertama.
      </P>
...
ALGORITMA
    ...
    ignoreBlank`}</Pseudocode>
      <P><strong>Pola penggunaan Versi 3:</strong></P>
      <Pseudocode>{`initAkses
while cc ≠ MARK do
    advKata
    { proses currentKata }`}</Pseudocode>
      <NoteBox>
        Perhatikan urutan: pada versi 3, <Mono>advKata</Mono> dipanggil <em>sebelum</em> proses,
        bukan setelah. <Mono>advKata</Mono> melakukan akuisisi kata ke <Mono>currentKata</Mono>,
        baru kemudian <Mono>currentKata</Mono> diproses.
      </NoteBox>
      <Divider />
      ...
      <W3Table
        ...
        rows={[
          ...
          ['Kondisi loop', 'not endKata', 'currentKata.length ≠ 0', 'cc ≠ MARK'],
        ]}
      />
      <div className="my-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-[13px]">
        {[
          { ver: 'Versi 1', pola: 'startKata\nwhile not endKata do\n    { proses }\n    advKata', color: 'bg-blue-50 border-blue-200 text-blue-900' },
          { ver: 'Versi 2', pola: 'startKata\nwhile length ≠ 0 do\n    { proses }\n    advKata', color: 'bg-purple-50 border-purple-200 text-purple-900' },
          { ver: 'Versi 3', pola: 'initAkses\nwhile cc ≠ MARK do\n    advKata\n    { proses }', color: 'bg-teal-50 border-teal-200 text-teal-900' },
        ].map((item) => (
          <div key={item.ver} className={`border rounded-xl px-4 py-3 ${item.color}`}>
            <p className="font-bold mb-2">{item.ver}</p>
            <pre className="font-mono text-[11px] whitespace-pre leading-relaxed opacity-90">{item.pola}</pre>
          </div>
        ))}
      </div>
      <Divider />
      ...
      <P>Hitung panjang rata-rata dari semua kata dalam pita. Jika pita kosong, output pesan khusus.</P>
      <P><strong>Menggunakan Versi 1:</strong></P>
      <Pseudocode>{`KAMUS
...
ALGORITMA
    ...
    if (nbKata ≠ 0) then
      ...
      <P><strong>Menggunakan Versi 3:</strong></P>
      <Pseudocode>{`KAMUS
    lengthTotal, nbKata: integer
...
ALGORITMA
    ...
    if (nbKata ≠ 0) then
      ...
      <P>Hitung berapa kali kata <Mono>&apos;WHILE&apos;</Mono> muncul dalam pita. Diperlukan fungsi pembantu:</P>
...
ALGORITMA
    ...
    output(nWHILE)`}</Pseudocode>
      ...
      <P>Karena program terdiri dari beberapa file, kompilasi dilakukan bersama:</P>
    ...
    <div className="text-[15px] text-gray-700">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          <span className="bg-yellow-100 text-yellow-700 text-xs font-bold px-2.5 py-1 rounded-full border border-yellow-200">Medium</span>
          <span className="bg-purple-50 text-purple-700 text-xs font-medium px-2.5 py-1 rounded-full border border-purple-100">Mesin Karakter</span>
          <span className="bg-purple-50 text-purple-700 text-xs font-medium px-2.5 py-1 rounded-full border border-purple-100">Traversal Pita</span>
          <span className="bg-purple-50 text-purple-700 text-xs font-medium px-2.5 py-1 rounded-full border border-purple-100">Pencacahan</span>
        </div>
        <h2 className="text-xl font-bold text-gray-900">Hitung-LE: Menghitung Pasangan &apos;L&apos; dan &apos;E&apos;</h2>
      </div>
      ...
      <div className="mb-5">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 leading-relaxed">
          <p>
            Diberikan sebuah mesin karakter dengan pita berisi karakter (mungkin kosong). Buatlah
            algoritma untuk menghitung banyaknya <strong>pasangan huruf &apos;L&apos; yang diikuti langsung
            oleh huruf &apos;E&apos;</strong> pada pita tersebut.
          </p>
      ...
      <div className="mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <div className="bg-gray-100 px-3 py-1.5 text-xs font-bold text-gray-500 border-b border-gray-200">Isi pita</div>
            <pre className="bg-white px-4 py-3 font-mono text-sm text-gray-800">{`HELLO WORLD LEVEL.`}</pre>
          </div>
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <div className="bg-gray-100 px-3 py-1.5 text-xs font-bold text-gray-500 border-b border-gray-200">Output</div>
            <pre className="bg-white px-4 py-3 font-mono text-sm text-gray-800">{`2\n{ "LE" di HELLO dan LEVEL }`}</pre>
          </div>
        </div>
        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <div className="bg-gray-100 px-3 py-1.5 text-xs font-bold text-gray-500 border-b border-gray-200">Pita kosong</div>
      ...
      <div className="mb-5 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-900">
        ...
        <code className="font-mono text-[13px]">start, adv, cc, eop, MARK = &apos;.&apos;</code>
      </div>
      ...
      <div className="mb-5">
        ...
        {showHint && (
          <div className="mt-2 bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-3 text-sm text-gray-700">
            <p className="mb-2">Gunakan variabel <code className="font-mono">prev</code> untuk menyimpan karakter sebelumnya:</p>
      ...
      <div>
        ...
        {showJawaban && (
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-800 mb-2">Ide Algoritma</h3>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-sm text-gray-700">
                <ol className="list-decimal list-inside space-y-1.5">
                  <li>Inisialisasi counter <code className="font-mono">ctr ← 0</code></li>
                  <li>Panggil <code className="font-mono">start</code> (First Elmt)</li>
                  <li>Dalam loop, simpan <code className="font-mono">cc</code> ke <code className="font-mono">prev</code>, lalu panggil <code className="font-mono">adv</code></li>
                  <li>Jika <code className="font-mono">prev = &apos;L&apos;</code> dan <code className="font-mono">cc = &apos;E&apos;</code>, tambah counter</li>
                  <li>Loop berakhir saat <code className="font-mono">cc = MARK</code></li>
                </ol>
              </div>
            </div>
...
ALGORITMA
    ...
    output(ctr)`}</Pseudocode>
            ...
            <div>
              <h3 className="text-base font-bold text-gray-800 mb-2">Trace untuk <code className="font-mono text-[14px]">HELLO WORLD LEVEL.</code></h3>
              <W3Table
            ...
            <div>
              <h3 className="text-base font-bold text-gray-800 mb-2">Kasus Pita Kosong</h3>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm text-gray-700">
                <p>Pita kosong = hanya berisi <code className="font-mono">&apos;.&apos;</code>.</p>
                <p className="mt-1">Setelah <code className="font-mono">start</code>, <code className="font-mono">cc = &apos;.&apos;</code> = MARK.</p>
                <p className="mt-1">Loop <code className="font-mono">while cc ≠ MARK</code> langsung tidak dieksekusi → output 0. ✓</p>
              </div>
            </div>
  ...
  if (fase === 'error') {
    return (
      <div className="py-10 text-center">
        ...
        <button onClick={() => generateSoal([])} className="px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700">Coba Lagi</button>
      </div>
    );
  }
      ...
      <div className="text-[15px] text-gray-700">
        ...
        <div className="border border-purple-200 rounded-xl p-4 text-center bg-purple-50">
          ...
          <p className="text-xs text-purple-600 mb-3">
            {konsepLemah.length > 0 ? `AI akan fokus pada: ${konsepLemah.slice(0, 3).join(', ')}${konsepLemah.length > 3 ? '...' : ''}` : 'AI akan membuat soal baru dengan tingkat kesulitan serupa'}
          </p>
          <button onClick={handleGenerateBaru} className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-lg transition-colors">Generate Soal Baru</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="text-[15px] text-gray-700">
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        ...
        {soal.notasiAlgoritma && (
          <div className="px-4 py-2.5 bg-gray-50 border-b border-gray-200">
            <button onClick={() => setShowNotasi((v) => !v)} className="text-xs text-purple-600 hover:underline font-medium flex items-center gap-1">
        ...
        {!currentFeedback && (
          <div className="px-4 py-3">
            <textarea
              ...
              placeholder={soal.tipe === 'implementasi' ? '// Tulis algoritma atau kode C kamu di sini...' : 'Tulis jawabanmu di sini...'}
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
        <button onClick={() => { setIdx(Math.max(0, idx - 1)); setShowNotasi(false); setEvalError(''); }} disabled={idx === 0} className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors min-h-10">← Sebelumnya</button>
        ...
        <div className="flex items-center gap-2">
          {!currentFeedback ? (
            <button onClick={evaluasiSoal} disabled={isEvaluating || !currentJawaban.trim()} className="flex items-center gap-2 px-5 py-2.5 bg-purple-600 hover:bg-purple-700 active:bg-purple-800 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg transition-colors min-h-10">
              {isEvaluating ? <><Spinner /> Menilai...</> : 'Nilai Soal Ini'}
            </button>
          ) : (
            <button onClick={handleLanjut} className="flex items-center gap-1.5 px-5 py-2.5 bg-purple-600 hover:bg-purple-700 active:bg-purple-800 text-white text-sm font-semibold rounded-lg transition-colors min-h-10">
...
function RingkasanContent() {
  return (
    <div className="text-[15px] text-gray-700">
      <div className="mb-5">
        <h2 className="text-xl font-bold text-gray-900">Ringkasan — Mesin Karakter dan Kata</h2>
        <p className="text-sm text-gray-400 mt-0.5">Mesin abstrak untuk pemrosesan pita</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        ...
        <div className="border border-purple-100 rounded-xl overflow-hidden">
          <div className="bg-purple-600 px-4 py-2.5 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">Komponen Mesin Karakter</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[13px]">
              {[
                { name: 'Pita', desc: 'Deret karakter diakhiri MARK = \'.\'', color: 'bg-blue-50 border-blue-100 text-blue-800' },
                { name: 'CC', desc: 'Current Character — isi jendela saat ini', color: 'bg-purple-50 border-purple-100 text-purple-800' },
                { name: 'EOP', desc: 'true jika CC = MARK; mesin berhenti', color: 'bg-red-50 border-red-100 text-red-800' },
                { name: 'start / adv', desc: 'First Elmt / Next Elmt pita', color: 'bg-teal-50 border-teal-100 text-teal-800' },
              ].map((item) => (
                <div key={item.name} className={`border rounded-lg px-3 py-2 ${item.color}`}>
                  <code className="font-mono font-bold text-[12px] block mb-0.5">{item.name}</code>
                  <div className="text-[11px] opacity-80">{item.desc}</div>
                </div>
              ))}
            </div>
            <div className="mt-3">
              <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Pola Pemrosesan</div>
    ...
    adv`}</pre>
        ...
        <div className="border border-indigo-100 rounded-xl overflow-hidden">
          <div className="bg-indigo-600 px-4 py-2.5 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">Primitif Bersama Mesin Kata</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[13px]">
              <div>
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">ignoreBlank</div>
                <pre className="bg-gray-900 text-green-300 rounded-lg px-3 py-2 font-mono text-[12px] leading-relaxed">{`while cc = BLANK do
    adv
{ cc ≠ BLANK }`}</pre>
              </div>
              <div>
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">salinKata</div>
...
length ← i`}</pre>
        ...
        <div className="border border-teal-100 rounded-xl overflow-hidden">
          <div className="bg-teal-600 px-4 py-2.5 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">3 Versi Model Akuisisi Kata</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-[12px]">
              {[
                { ver: 'Versi 1', penanda: 'endKata (boolean)', init: 'startKata', pola: 'while not endKata do\n    { proses }\n    advKata', color: 'bg-blue-50 border-blue-100 text-blue-900' },
                { ver: 'Versi 2', penanda: 'length = 0', init: 'startKata', pola: 'while length ≠ 0 do\n    { proses }\n    advKata', color: 'bg-purple-50 border-purple-100 text-purple-900' },
                { ver: 'Versi 3', penanda: 'cc = MARK', init: 'initAkses', pola: 'while cc ≠ MARK do\n    advKata\n    { proses }', color: 'bg-teal-50 border-teal-100 text-teal-900' },
              ].map((item) => (
                <div key={item.ver} className={`border rounded-xl px-3 py-3 ${item.color}`}>
                  <p className="font-bold mb-1">{item.ver}</p>
                  <p className="text-[11px] mb-0.5 opacity-70">Penanda: <strong>{item.penanda}</strong></p>
                  <p className="text-[11px] mb-2 opacity-70">Init: <code className="font-mono">{item.init}</code></p>
                  <pre className="font-mono text-[10px] leading-relaxed opacity-90 bg-white bg-opacity-50 rounded px-2 py-1">{item.pola}</pre>
                </div>
              ))}
            </div>
            <div className="mt-3 bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 text-[12px] text-yellow-800">
              <strong>Perhatian Versi 3:</strong> <code className="font-mono">advKata</code> dipanggil <em>sebelum</em> proses, bukan sesudah.
            </div>
          </div>
        </div>
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          <div className="bg-gray-800 px-4 py-2.5 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">Struktur File &amp; Kompilasi</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[12px]">
              <div>
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Struktur File</div>
...
mainkata.c`}</pre>
              ...
              <div>
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Kompilasi</div>
  ...
  mainkata.c`}</pre>
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
        <p className="text-sm font-bold text-purple-800 mb-2">Kapan Menggunakan Versi Mana?</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-[12px]">
          {[
            { ver: 'Versi 1', when: 'Perlu state eksplisit untuk kontrol alur lebih fleksibel', color: 'bg-white border-purple-100 text-purple-900' },
            { ver: 'Versi 2', when: 'Ingin menghindari state boolean tambahan; sentinel lebih bersih', color: 'bg-white border-purple-100 text-purple-900' },
            { ver: 'Versi 3', when: 'Paling sederhana; langsung pakai cc sebagai kondisi loop', color: 'bg-white border-purple-100 text-purple-900' },
          ].map((item) => (
            <div key={item.ver} className={`border rounded-lg px-3 py-2 ${item.color}`}>
  ...
  return (
    <div className="flex flex-col lg:flex-row overflow-hidden" style={{ height: 'calc(100vh - 56px)' }}>
      ...
      <aside className="hidden lg:block w-56 shrink-0 bg-white border-r border-gray-200 overflow-y-auto">
        <div className="py-3">
          <div className="px-4 py-2 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Mesin Karakter & Kata</div>
          {SECTIONS.map((s) => (
            <button
              key={s.id}
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-t-lg transition-colors flex items-center gap-1.5 ${isActive ? 'bg-purple-700 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  {tab}
                  {isDone && (
          ...
          <div className="lg:hidden shrink-0 bg-gray-50 border-b border-gray-200">
            <button onClick={() => setShowToc((v) => !v)} className="w-full flex items-center justify-between px-4 py-2.5 text-sm font-medium text-gray-700 active:bg-gray-100">
              <span className="flex items-center gap-2">
                <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h10" /></svg>
                Daftar Isi
              </span>
              <svg className={`w-4 h-4 text-gray-400 transition-transform ${showToc ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </button>
            {showToc && (
              <div className="max-h-52 overflow-y-auto border-t border-gray-100 bg-white">
                {SECTIONS.map((s) => (
                  <button key={s.id} onClick={() => { scrollToSection(s.id); setShowToc(false); }} className={`w-full text-left text-[13px] px-4 py-2 transition-colors ${s.level === 1 ? 'pl-8' : ''} ${activeSection === s.id ? 'bg-purple-50 text-purple-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}>
                    {s.title}
                  </button>
                ))}
              </div>
        ...
        <div ref={mainRef} className="flex-1 overflow-y-auto">
          <div className="max-w-3xl mx-auto px-4 sm:px-8 py-5 sm:py-6">
            ...
            <div className="mt-10 pt-6 border-t border-gray-200 flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-0 justify-between">
              <p className="text-sm text-gray-400">{Object.values(completed).filter(Boolean).length} dari 4 seksi diselesaikan</p>
              {completed[TAB_KEYS[activeTab]] ? (
                ...
                <button onClick={() => handleComplete(activeTab)} className="w-full sm:w-auto px-5 py-2.5 sm:py-2 bg-purple-600 hover:bg-purple-700 active:bg-purple-800 text-white text-sm font-semibold rounded-lg transition-colors">
                  Tandai Selesai
                </button>
              )}
            </div>
