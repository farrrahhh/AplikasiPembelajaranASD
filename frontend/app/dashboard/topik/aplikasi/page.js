...
export default function AplikasiPage() {
  ...
  useEffect(() => {
    fetchTopicProgress('aplikasi').then((prog) => {
  ...
  const handleComplete = (tab) => {
    ...
    setCompleted(next);
    saveTopicProgress('aplikasi', next);
  };

  const handleQuestionEvaluated = useCallback((questionId) => {
    try {
      const evaluated = new Set(JSON.parse(localStorage.getItem('asd_evaluated_aplikasi') ?? '[]'));
      evaluated.add(questionId);
      localStorage.setItem('asd_evaluated_aplikasi', JSON.stringify([...evaluated]));
    } catch {}
  }, []);
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-t-lg transition-colors flex items-center gap-1.5 ${isActive ? 'bg-orange-700 text-white' : isLocked ? 'bg-gray-50 text-gray-300 cursor-not-allowed' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  {tab}
                  {isLocked && (
