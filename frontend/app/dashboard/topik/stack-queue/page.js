...
type Queue: < buffer: array [0..CAPACITY-1] of ElType,
      ...
      <P>
        HEAD selalu di indeks 0. Saat <Mono>dequeue</Mono>, <strong>semua elemen digeser ke kiri</strong>.
      </P>
...
4. Kasus khusus (berelemen 1): idxHead dan idxTail = IDX_UNDEF`}</Pseudocode>
      <InfoBox>
        <strong>Analisis Alt-1:</strong> Sederhana karena HEAD selalu di indeks 0,
        tetapi <em>tidak efisien</em> — setiap dequeue membutuhkan O(n) operasi pergeseran.
      </InfoBox>
      <Divider />
...
3. Jika lebih dari 1: idxHead++`}</Pseudocode>
      <NoteBox>
        <strong>Masalah penuh semu (semi-full):</strong> Ketika <Mono>idxTail = CAPACITY-1</Mono>{' '}
        tetapi masih ada ruang kosong di kiri. Solusinya: saat akan enqueue dan idxTail sudah
        di ujung kanan, geser semua elemen kembali ke indeks 0.
      </NoteBox>
      <Divider />

      {/* ── 7. Alt-3 Circular ────────────────────────────────── */}
      <SectionHeading id="queue-alt3">Implementasi Queue: Alt-3 (Circular Buffer)</SectionHeading>
      <P>
        HEAD dan TAIL berputar mengelilingi array — setelah mencapai ujung kanan, kembali
        ke awal. <strong>Tidak perlu pergeseran apapun</strong> — paling efisien.
      </P>
...
Ringkas: idxTail baru = (idxTail + 1) mod CAPACITY`}</Pseudocode>
      <P>Variasi lain: mengganti <Mono>idxTail</Mono> dengan <Mono>count</Mono> (jumlah elemen saat ini) sehingga kondisi penuh cukup dicek dengan <Mono>count = CAPACITY</Mono>.</P>
      <InfoBox>
        <strong>Analisis Alt-3:</strong> Enqueue dan dequeue keduanya O(1) — tidak ada
        pergeseran elemen sama sekali. Ini adalah implementasi terbaik untuk queue berbasis array.
      </InfoBox>
      <Divider />

      {/* ── 8. Implementasi C ────────────────────────────────── */}
      <SectionHeading id="queue-c">Implementasi Queue dalam Bahasa C (Alt-2)</SectionHeading>
...
`}</CodeBlock>
      ...
      <SubHeading>Implementasi enqueue (Alt-2 — geser jika mentok kanan)</SubHeading>
      <CodeBlock language="c">{`
void enqueue(Queue *q, ElType val) {
    if (isEmpty(*q)) {
        IDX_HEAD(*q) = 0;
...
TOP = IDX_UNDEF`}</AsciiBox>
      ...
      {/* ── 11. Perbandingan ─────────────────────────────────── */}
      <SectionHeading id="stack-perbandingan">Queue vs Stack vs List</SectionHeading>
      <W3Table
        headers={['Konsep', 'Prinsip', 'Ujung Aktif']}
        rows={[
          ['Stack', 'LIFO — terakhir masuk, pertama keluar', 'Satu ujung (TOP)'],
          ['Queue', 'FIFO — pertama masuk, pertama keluar',  'Dua ujung (HEAD dan TAIL)'],
...
type Stack: < buffer: array [0..CAPACITY-1] of ElType,
      ...
      <P>
        Stack kosong ditandai dengan <Mono>idxTop = IDX_UNDEF</Mono>.
        Stack tidak memerlukan <Mono>idxHead</Mono> karena hanya beroperasi di satu ujung.
      </P>
      <Divider />

      {/* ── 14. Push & Pop ───────────────────────────────────── */}
      <SectionHeading id="stack-pushpop">Algoritma Push & Pop</SectionHeading>
      <P>Algoritma <strong>push</strong> (tambah elemen ke TOP):</P>
      <Pseudocode>{`s.idxTop ← s.idxTop + 1
s.buffer[s.idxTop] ← val`}</Pseudocode>
      <P>Algoritma <strong>pop</strong> (ambil elemen dari TOP):</P>
      <Pseudocode>{`val ← top(s)        { yaitu s.buffer[s.idxTop] }
s.idxTop ← s.idxTop - 1`}</Pseudocode>
      <InfoBox>
        <strong>Mengapa Stack lebih sederhana dari Queue?</strong> Stack hanya mengelola
        satu ujung (TOP) sehingga push dan pop masing-masing O(1) tanpa pergeseran dan
        tanpa masalah penuh semu.
      </InfoBox>
      <Divider />

      {/* ── 15. Implementasi C Stack ─────────────────────────── */}
      <SectionHeading id="stack-c">Implementasi Stack dalam Bahasa C</SectionHeading>
...
`}</CodeBlock>
      ...
      {/* ── 16. Aplikasi Postfix ─────────────────────────────── */}
      <SectionHeading id="aplikasi">Aplikasi: Evaluasi Ekspresi Postfix</SectionHeading>
      <P>
        Stack sangat cocok untuk mengevaluasi ekspresi <strong>postfix</strong>{' '}
        (Reverse Polish Notation — RPN). Dibandingkan dengan notasi infix, postfix
...
Selesai → output: (A×B)/C`}</AsciiBox>
      ...
      <SectionHeading id="tabel-perbandingan">Tabel Perbandingan Queue vs Stack</SectionHeading>
      <W3Table
        headers={['Aspek', 'Queue', 'Stack']}
        rows={[
          ['Prinsip',           'FIFO',                       'LIFO'],
          ['Ujung aktif',       'HEAD (keluar) & TAIL (masuk)', 'TOP (masuk & keluar)'],
          ...
          ['Alt. implementasi', 'Alt-1, Alt-2, Alt-3 (circular)', 'Satu cara (TOP naik/turun)'],
          ['Contoh penggunaan', 'Printer, job scheduling',      'Undo, call stack, postfix'],
        ]}
      />

      {/* Summary card */}
      <div className="mt-10 mb-6 bg-rose-50 border border-rose-200 rounded-xl p-6">
        <h3 className="font-bold text-rose-900 text-base mb-3">Ringkasan Penting</h3>
  ...
  return (
    <div className="text-[15px] text-gray-700">
      ...
      <div className="mb-6">
        ...
        <h2 className="text-xl font-bold text-gray-900">Evaluasi Ekspresi Postfix</h2>
      </div>
      ...
      <div className="mb-5">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 leading-relaxed">
          <p>
            ...
            <code className="bg-white border border-gray-200 text-rose-700 px-1.5 rounded font-mono text-[13px]">2 3 + 4 *</code>
          </p>
          <p className="mt-2">
            Dengan menggunakan ADT Stack, tuliskan penelusuran (trace) langkah demi langkah
            evaluasi ekspresi postfix tersebut. Kemudian tulis pseudocode algoritma umum
            untuk mengevaluasi ekspresi postfix sembarang.
          </p>
          <p className="mt-2 text-sm text-gray-500">
            Aturan: jika token adalah operan (angka) → push ke stack;
            jika operator → pop dua operan, hitung, push hasilnya.
          </p>
        </div>
      </div>
      ...
      <div className="mb-5">
        ...
        {showHint && (
          <div className="mt-2 bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-3 text-sm text-gray-700">
            <p className="mb-2">Bayangkan kamu membaca token dari kiri ke kanan:</p>
            <ul className="list-disc list-inside space-y-1 text-sm">
              <li>Token <code className="font-mono">2</code> → angka, push ke stack</li>
              <li>Token <code className="font-mono">3</code> → angka, push ke stack</li>
              <li>Token <code className="font-mono">+</code> → operator, pop dua angka teratas, tambahkan, push hasilnya</li>
              <li>Token <code className="font-mono">4</code> → angka, push ke stack</li>
              <li>Token <code className="font-mono">*</code> → operator, pop dua angka teratas, kalikan, push hasilnya</li>
            </ul>
            <p className="mt-2">Saat semua token habis, top(stack) adalah hasil akhir.</p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        {showJawaban && (
          <div className="space-y-6">
            ...
            {/* Trace table */}
            <div>
              <h3 className="text-base font-bold text-gray-800 mb-2">Trace Langkah demi Langkah</h3>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      <th className="bg-gray-800 text-white px-4 py-2.5 text-left font-semibold border-r border-gray-600">Token</th>
                      <th className="bg-gray-800 text-white px-4 py-2.5 text-left font-semibold border-r border-gray-600">Aksi</th>
                      <th className="bg-gray-800 text-white px-4 py-2.5 text-left font-semibold">Stack (bawah → atas)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-4 py-2.5 font-mono font-bold text-rose-700 border-t border-gray-100 border-r">{token}</td>
                        <td className="px-4 py-2.5 text-gray-700 border-t border-gray-100 border-r">{aksi}</td>
                        <td className="px-4 py-2.5 font-mono text-gray-700 border-t border-gray-100">{stack}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-2 bg-green-50 border border-green-200 rounded-lg px-4 py-2.5 text-sm text-green-800">
                ...
                <code className="font-mono">top(stack) = 20</code> — sesuai dengan (2+3)×4 = 20
              </div>
            </div>

            {/* Pseudocode */}
            <div>
              <h3 className="text-base font-bold text-gray-800 mb-2">Pseudocode Algoritma Umum</h3>
...
ALGORITMA
  ...
  output ← top(s)`}
            ...
            {/* Implementasi C */}
            <div>
              <h3 className="text-base font-bold text-gray-800 mb-2">Implementasi Inti dalam C</h3>
...
if (isdigit(token[0])) {
    ...
    push(&s, applyOp(token[0], a, b));
              ...
              <div className="mt-3 bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-2.5 text-sm text-yellow-800">
                <strong>Perhatian urutan pop:</strong> Pop pertama menghasilkan operan
                <em> kedua</em> (b), pop kedua menghasilkan operan <em>pertama</em> (a).
                Untuk operasi tidak komutatif seperti <code className="font-mono">-</code> dan <code className="font-mono">/</code>,
                urutan ini penting: hasilnya adalah <code className="font-mono">a op b</code>, bukan <code className="font-mono">b op a</code>.
              </div>
            </div>
  ...
  if (fase === 'loading') {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-gray-500">
        <svg className="animate-spin w-8 h-8 mb-4 text-rose-500" viewBox="0 0 24 24" fill="none">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
        </svg>
      ...
      <div className="text-[15px] text-gray-700">
        ...
        <div className="bg-gradient-to-r from-rose-600 to-rose-700 rounded-xl p-5 text-white mb-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-rose-200 text-sm font-medium">Nilai Rata-rata</p>
              <p className="text-4xl font-bold">{avgSkor}<span className="text-xl text-rose-300">/100</span></p>
            </div>
        ...
        <div className="flex items-center gap-2 mb-5">
          {soalList.map((sq, i) => {
            ...
            else cls += i === resultIdx ? 'bg-white border-rose-600 text-rose-600' : 'bg-gray-100 border-gray-300 text-gray-400';
            return (
        ...
        {curSoal && curFb && (
          <div className="border border-gray-200 rounded-xl overflow-hidden mb-4">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-rose-600 text-white flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{curSoal.id}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${curSoal.tipe === 'implementasi' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-teal-50 text-teal-700 border-teal-200'}`}>
    ...
    <div className="text-[15px] text-gray-700">
      ...
      <div className="flex items-center gap-2 mb-5">
        {soalList.map((sq, i) => {
          ...
          let cls = 'border-2 ';
          if (f) cls += f.skor >= 70 ? 'bg-green-500 border-green-500 text-white' : 'bg-red-400 border-red-400 text-white';
          else if (isCur) cls += jawaban[sq.id]?.trim() ? 'bg-rose-600 border-rose-600 text-white' : 'bg-white border-rose-600 text-rose-600';
          else cls += jawaban[sq.id]?.trim() ? 'bg-rose-100 border-rose-400 text-rose-700' : 'bg-gray-100 border-gray-300 text-gray-400';
          return (
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-start gap-3">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 ${currentFeedback ? (currentFeedback.skor >= 70 ? 'bg-green-500 text-white' : 'bg-red-400 text-white') : (currentJawaban.trim() ? 'bg-rose-600 text-white' : 'bg-gray-200 text-gray-500')}`}>
            {currentJawaban.trim() && !currentFeedback ? '✓' : soal.id}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${soal.tipe === 'implementasi' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-teal-50 text-teal-700 border-teal-200'}`}>
...
function RingkasanContent() {
  return (
    <div className="text-[15px] text-gray-700">
      ...
      <div className="mb-5">
        <h2 className="text-xl font-bold text-gray-900">Ringkasan — Queue dan Stack</h2>
        <p className="text-sm text-gray-400 mt-0.5">Implementasi Queue & Stack dengan Array</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        ...
        <div className="border border-rose-100 rounded-xl overflow-hidden">
          <div className="bg-rose-600 px-4 py-2.5 flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-white text-rose-600 text-xs font-bold flex items-center justify-center shrink-0">1</span>
            <span className="text-white font-bold text-sm">Queue — FIFO</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[13px]">
              {[
                { name: 'Alt-1', desc: 'HEAD di indeks 0, geser semua saat dequeue', color: 'bg-red-50 border-red-100 text-red-800', perf: 'O(n) dequeue' },
                { name: 'Alt-2', desc: 'HEAD geser kanan, geser balik jika mentok kanan', color: 'bg-yellow-50 border-yellow-100 text-yellow-800', perf: 'O(n) worst case' },
                { name: 'Alt-3', desc: 'Circular buffer, modular index', color: 'bg-green-50 border-green-100 text-green-800', perf: 'O(1) selalu' },
              ].map((item) => (
                <div key={item.name} className={`border rounded-lg px-3 py-2 ${item.color}`}>
                  ...
                  <code className="text-[11px] font-mono font-bold opacity-90">{item.perf}</code>
                </div>
              ))}
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 text-[13px]">
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Struktur</div>
                <code className="font-mono text-[12px]">{'Queue: <buffer, idxHead, idxTail>'}</code>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Kosong</div>
                <code className="font-mono text-[12px]">idxHead = idxTail = IDX_UNDEF</code>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border border-orange-100 rounded-xl overflow-hidden">
          <div className="bg-orange-600 px-4 py-2.5 flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-white text-orange-600 text-xs font-bold flex items-center justify-center shrink-0">2</span>
          ...
          <div className="px-4 py-3">
            <div className="grid grid-cols-2 gap-2 text-[13px] mb-3">
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Struktur</div>
                <code className="font-mono text-[12px]">{'Stack: <buffer, idxTop>'}</code>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Kosong</div>
                <code className="font-mono text-[12px]">idxTop = IDX_UNDEF</code>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 text-[13px]">
              <div className="bg-orange-50 border border-orange-100 text-orange-800 rounded-lg px-3 py-2">
                ...
                <code className="text-[11px] font-mono">idxTop++; buffer[idxTop] = val</code>
              </div>
              <div className="bg-orange-50 border border-orange-100 text-orange-800 rounded-lg px-3 py-2">
                <div className="font-bold mb-0.5">pop</div>
                <code className="text-[11px] font-mono">val = buffer[idxTop]; idxTop--</code>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border border-indigo-100 rounded-xl overflow-hidden">
          <div className="bg-indigo-600 px-4 py-2.5 flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-white text-indigo-600 text-xs font-bold flex items-center justify-center shrink-0">3</span>
            <span className="text-white font-bold text-sm">Aplikasi: Evaluasi Postfix</span>
          </div>
          <div className="px-4 py-3 text-sm text-gray-700">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Algoritma</div>
...
output: top(stack)`}</pre>
              ...
              <div>
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Trace: 2 3 + 4 *</div>
...
Hasil: 20`}</pre>
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          <div className="bg-gray-800 px-4 py-2.5 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">Perbandingan Cepat</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 gap-2 text-[13px]">
              {[
                { aspek: 'Prinsip',       queue: 'FIFO',          stack: 'LIFO',           color: 'bg-blue-50 border-blue-100 text-blue-800' },
                { aspek: 'Pointer',       queue: 'HEAD + TAIL',    stack: 'TOP saja',       color: 'bg-green-50 border-green-100 text-green-800' },
                { aspek: 'Tambah',        queue: 'enqueue → TAIL', stack: 'push → TOP',     color: 'bg-yellow-50 border-yellow-100 text-yellow-800' },
                { aspek: 'Hapus',         queue: 'dequeue ← HEAD', stack: 'pop ← TOP',      color: 'bg-orange-50 border-orange-100 text-orange-800' },
              ].map((row) => (
                <div key={row.aspek} className={`border rounded-lg px-3 py-2 ${row.color}`}>
                  <span className="font-bold mr-2">{row.aspek}:</span>
                  <span className="mr-2">Queue = <code className="font-mono">{row.queue}</code></span>
                  <span>Stack = <code className="font-mono">{row.stack}</code></span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 mb-6">
        <p className="text-sm font-bold text-rose-800 mb-2">Operasi Queue — Referensi Cepat</p>
...
length      → jumlah elemen`}</pre>
      ...
      <div className="border border-gray-200 rounded-xl p-4 bg-gray-50">
        <p className="text-sm font-bold text-gray-700 mb-3">Kondisi Khusus yang Perlu Diingat</p>
        <div className="grid grid-cols-1 gap-2 text-[13px]">
          {[
            { tip: 'Queue kosong', detail: 'idxHead = IDX_UNDEF dan idxTail = IDX_UNDEF', color: 'bg-rose-50 border-rose-100 text-rose-800' },
            { tip: 'Stack kosong', detail: 'idxTop = IDX_UNDEF (yaitu -1)', color: 'bg-orange-50 border-orange-100 text-orange-800' },
            { tip: 'Alt-2 semi-full', detail: 'idxTail = CAPACITY-1 tapi masih ada ruang di kiri → perlu geser balik', color: 'bg-yellow-50 border-yellow-100 text-yellow-800' },
            { tip: 'Postfix pop order', detail: 'Pop pertama = operan kedua (b), pop kedua = operan pertama (a)', color: 'bg-blue-50 border-blue-100 text-blue-800' },
          ].map((item) => (
              ...
              return (
                <button
                  ...
                  className={`shrink-0 px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-t-lg transition-colors flex items-center gap-1.5 ${
                    isActive ? 'bg-rose-700 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                >
                  {tab}
                  {isDone && (
                ...
                {SECTIONS.map((s) => (
                  <button
                    ...
                    className={`w-full text-left text-[13px] px-4 py-2 transition-colors ${
                      ...
                      activeSection === s.id ? 'bg-rose-50 text-rose-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'
                    }`}
                  >
                    {s.title}
                  </button>
