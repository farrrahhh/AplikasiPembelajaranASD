    ...
    <div className="my-4 overflow-x-auto rounded-lg border border-gray-200">
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr>
            {headers.map((h, i) => (
              <th key={i} className="bg-gray-800 text-white px-4 py-2.5 text-left font-semibold border-r border-gray-600 last:border-r-0">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
              {row.map((cell, j) => (
                <td key={j} className="px-4 py-2.5 text-gray-700 border-t border-gray-100 border-r last:border-r-0">
                  {cell}
                </td>
              ))}
            </tr>
...
add(S, 26) → S = {7, 12, 22, 26, 32, 47}  ✗ (26 sudah ada, tidak ditambah)`}</AsciiBox>
      ...
      <P>Jika <Mono>S</Mono>, <Mono>S1</Mono>, <Mono>S2</Mono> adalah Set dengan elemen <Mono>ElmtS</Mono>:</P>
      <W3Table
...
9) isEmpty(add(v, S))   = false`}</Pseudocode>
      <InfoBox>
        Aksioma 6 berarti <Mono>remove(v, add(v, S))</Mono> mengembalikan <Mono>remove(v, S)</Mono>
        — penghapusan <em>semua</em> kemunculan v (meskipun dalam Set, v sudah unik sehingga hanya ada satu).
        Aksioma 4 dan 7 menjamin bahwa operasi pada elemen lain tidak berpengaruh.
      </InfoBox>
      <Divider />

      {/* ── 4. Implementasi Array ────────────────────────────────── */}
      <SectionHeading id="set-array">Implementasi Set dengan Array</SectionHeading>
...
2. Berdasarkan urutan insert (tidak terurut)
   → [12, 26, 7, 47, 32]`}</AsciiBox>
      ...
      <P>
        Untuk <Mono>remove</Mono> pada array tidak terurut, elemen terakhir dipindahkan
        ke posisi elemen yang dihapus — tidak perlu menggeser semua elemen:
      </P>
...
Elemen terakhir (32) dipindah ke posisi 26 (indeks 1).`}</AsciiBox>
      ...
      <W3Table
        ...
        rows={[
          ...
          ['union',  'O(n × m) — cek satu per satu', 'O(n + m) — merge dua list'],
        ]}
      />
      <InfoBox>
        <strong>Kapan memilih mana?</strong> Jika <Mono>isIn</Mono> jauh lebih sering dipanggil
        daripada <Mono>add</Mono>, gunakan array <strong>terurut</strong> (binary search O(log n)).
        Jika <Mono>add</Mono> lebih dominan, gunakan array <strong>tidak terurut</strong> (O(1)).
        Untuk kinerja terbaik secara keseluruhan, gunakan hash table (O(1) rata-rata).
      </InfoBox>
      <Divider />
      ...
      <P>
        <strong>Map</strong> adalah kumpulan pasangan <Mono>{'<key, value>'}</Mono> di mana
        nilai <Mono>key</Mono> bersifat unik. Map juga dikenal sebagai:
      </P>
...
+------------+-----------+`}</AsciiBox>
      ...
      <W3Table
        ...
        rows={[
          ...
          ['find(K, M)',   'Mengembalikan value dari key K, atau VAL_UNDEF jika tidak ada'],
...
6) unset(k, set(j, v, M))   = set(j, v, unset(k, M))  jika k ≠ j`}</Pseudocode>
      <InfoBox>
        Aksioma 2 menjamin bahwa <Mono>find</Mono> setelah <Mono>set</Mono> selalu mengembalikan
        value yang baru saja di-set. Aksioma 3 menjamin bahwa operasi pada key lain tidak
        berpengaruh pada key yang sedang dicari.
      </InfoBox>
      <Divider />

      {/* ── 9. Implementasi Map Array ────────────────────────────── */}
      <SectionHeading id="map-array">Implementasi Map dengan Array</SectionHeading>
      <Pseudocode>{`KAMUS UMUM
constant CAPACITY: integer = 100
...
Value:  <"nama","Budi"> <"nim","0001"> <"jur","IF">
                                        length = 3`}</AsciiBox>
      <NoteBox>
        Pada implementasi array biasa, <Mono>find</Mono> membutuhkan O(n) untuk mencari key.
        Untuk performa lebih baik, gunakan hash table yang memetakan key ke indeks array secara langsung.
      </NoteBox>
      <Divider />
      ...
      <P>
        <strong>Fungsi hash</strong> adalah fungsi yang memetakan data berukuran berapa pun
        menjadi nilai berukuran tetap (indeks array).
      </P>
      ...
      <P>
        <strong>Collision</strong> terjadi ketika dua key berbeda menghasilkan nilai hash yang sama.
      </P>
...
Sandra Dee  ─────────────────→   02  ← collision dengan Lisa Smith!`}</AsciiBox>
      <P>Collision tidak bisa dihindari sepenuhnya (pigeonhole principle), sehingga perlu strategi penyelesaian.</P>
      <Divider />

      {/* ── 12. Hash Chaining ────────────────────────────────────── */}
      <SectionHeading id="hash-chaining">Strategi 1: Hash Chaining</SectionHeading>
      <P>
        Setiap slot menyimpan sebuah <strong>linked list</strong>. Jika terjadi collision,
        elemen baru ditambahkan ke list pada slot yang sama.
      </P>
...
Slot 254: → [Sam Doe    | 521-5030] → NULL`}</AsciiBox>
      ...
      {/* ── 13. Open Addressing ──────────────────────────────────── */}
      <SectionHeading id="hash-probing">Strategi 2: Open Addressing</SectionHeading>
      <P>
        Jika slot terisi, cari slot lain yang kosong dalam array yang sama.
        Tiga metode pencarian slot alternatif:
      </P>
      <W3Table
        headers={['Metode', 'Cara Mencari Slot Alternatif', 'Masalah']}
        rows={[
          ['Linear Probing',    '+1, +2, +3, ... (modular)',      'Clustering — slot bergerombol'],
          ['Quadratic Probing', '+1, +4, +9, ... (modular)',      'Secondary clustering'],
          ['Double Hashing',    'Gunakan fungsi hash kedua',      'Lebih kompleks'],
        ]}
      />
      <P>Contoh linear probing dengan <Mono>hash(k) = k mod 7</Mono>:</P>
...
hash("Sam")  = 6 → slot 6 terisi → cek slot 0 → kosong → simpan di slot 0`}</AsciiBox>
      ...
      <P><strong>Load factor</strong> adalah rasio slot terisi terhadap total slot:</P>
      ...
      <NoteBox>
        Semakin tinggi load factor, semakin besar kemungkinan collision dan semakin
        panjang probe sequence, sehingga kinerja mendekati O(n).
      </NoteBox>
      <Divider />

      {/* ── 15. Algoritma set & find ─────────────────────────────── */}
      <SectionHeading id="hash-algoritma">Algoritma set & find pada Map dengan Hash</SectionHeading>
...
ALGORITMA
  ...
  if found then
      ...
      {/* ── 16. Penghapusan ──────────────────────────────────────── */}
      <SectionHeading id="hash-delete">Penghapusan Elemen pada Hash Table</SectionHeading>
      <P>
        Menghapus elemen di indeks <Mono>i</Mono> tidak bisa hanya mengosongkan slot tersebut.
      </P>
      <InfoBox>
        <strong>Masalahnya:</strong> Pencarian berhenti saat menemukan slot kosong. Jika ada elemen
        lain yang dipindah akibat collision ke slot setelah <Mono>i</Mono>, elemen tersebut tidak
        akan ditemukan lagi karena pencarian berhenti lebih awal di slot yang sudah dikosongkan.
      </InfoBox>
      <P>Solusinya: setelah menghapus elemen di indeks <Mono>i</Mono>, pindahkan elemen-elemen
        setelahnya yang hash aslinya ≤ <Mono>i</Mono> ke posisi yang lebih dekat dengan hash aslinya:</P>
...
ALGORITMA
  ...
  while m.buffer[j] ≠ NIL do
      ...
      <SectionHeading id="tabel-perbandingan">Tabel Perbandingan Implementasi</SectionHeading>
      <W3Table
      ...
      <InfoBox>
        ...
        dengan syarat <strong>load factor dijaga rendah</strong> (idealnya ≤ 0.7) dan
        collision dikelola dengan baik.
      </InfoBox>

      {/* Summary card */}
      <div className="mt-10 mb-6 bg-teal-50 border border-teal-200 rounded-xl p-6">
        <h3 className="font-bold text-teal-900 text-base mb-3">Ringkasan Penting</h3>
  ...
  return (
    <div className="text-[15px] text-gray-700">
      ...
      <div className="mb-6">
        ...
        <h2 className="text-xl font-bold text-gray-900">Simulasi Hash Table dengan Linear Probing</h2>
      </div>
      ...
      <div className="mb-5">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 leading-relaxed">
          <p>
            Diberikan Map berbasis hash table dengan <strong>CAPACITY = 7</strong> dan
            fungsi hash: <code className="font-mono bg-white border border-gray-200 text-teal-700 px-1.5 rounded text-[13px]">hash(k) = k mod 7</code>.
            Semua slot awalnya kosong (NIL).
          </p>
          <p className="mt-2">
            Simulasikan operasi <code className="font-mono text-teal-700">set</code> berikut secara berurutan,
            gambarkan keadaan hash table setelah semua operasi selesai, dan tunjukkan langkah penanganan
            collision jika ada:
          </p>
      ...
      <div className="mb-5">
        ...
        {showHint && (
          <div className="mt-2 bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-3 text-sm text-gray-700">
            ...
            <p className="mt-2">Jika slot hash sudah terisi, geser +1 (linear probing) sampai menemukan slot kosong.</p>
          </div>
        )}
      </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {[
                  { op: 'set(10, "A")', hash: 'hash(10) = 3', slot: 'Slot 3 kosong → simpan di slot 3', collision: false },
                  { op: 'set(3, "B")',  hash: 'hash(3) = 3',  slot: 'Slot 3 terisi (10) → probe slot 4 → kosong → simpan di slot 4', collision: true },
                  { op: 'set(17, "C")', hash: 'hash(17) = 3', slot: 'Slot 3 terisi → slot 4 terisi → slot 5 kosong → simpan di slot 5', collision: true },
                  { op: 'set(14, "D")', hash: 'hash(14) = 0', slot: 'Slot 0 kosong → simpan di slot 0', collision: false },
                  { op: 'set(6, "E")',  hash: 'hash(6) = 6',  slot: 'Slot 6 kosong → simpan di slot 6', collision: false },
                ].map((step, i) => (
                  <div key={i} className={`border rounded-lg p-3 ${step.collision ? 'border-orange-200 bg-orange-50' : 'border-green-200 bg-green-50'}`}>
                    <div className="flex items-start gap-2">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full shrink-0 mt-0.5 ${step.collision ? 'bg-orange-200 text-orange-700' : 'bg-green-200 text-green-700'}`}>
                        {step.collision ? 'Collision' : 'OK'}
                      </span>
                      <div>
                        <code className="font-mono font-bold text-[13px] text-gray-800">{step.op}</code>
                        <p className="text-xs text-gray-500 mt-0.5"><code className="font-mono">{step.hash}</code></p>
                        <p className="text-sm text-gray-700 mt-0.5">{step.slot}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {/* Final table */}
            <div>
              <h3 className="text-base font-bold text-gray-800 mb-2">Keadaan Akhir Hash Table</h3>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      {[0,1,2,3,4,5,6].map((i) => (
                        <th key={i} className="bg-gray-800 text-white px-4 py-2.5 text-center font-semibold border-r border-gray-600 last:border-r-0">
                          [{i}]
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    <tr className="bg-gray-50">
                      {[
                        ...
                        <td key={i} className={`px-3 py-2.5 text-center border-t border-gray-100 border-r last:border-r-0 ${cell.collision ? 'bg-orange-50' : ''}`}>
                          {cell.used ? (
                            <>
                              <div className="font-mono font-bold text-teal-700 text-[13px]">{cell.key}</div>
                              <div className="font-mono text-gray-600 text-[12px]">{cell.val}</div>
                            </>
                          ) : (
                            <span className="text-gray-300 text-[12px] font-mono">NIL</span>
                          )}
                        </td>
                      ))}
                    </tr>
                    <tr>
                      {[
                        'hash=0', '—', '—', 'hash=3', 'hash=3*', 'hash=3*', 'hash=6'
                      ].map((note, i) => (
                        <td key={i} className="px-3 py-1.5 text-center border-t border-gray-100 border-r last:border-r-0">
                          <span className={`text-[10px] font-mono ${note.includes('*') ? 'text-orange-500' : 'text-gray-400'}`}>{note}</span>
                        </td>
                      ))}
                    </tr>
                  </tbody>
              ...
              <p className="text-xs text-orange-600 mt-1.5">* Elemen dipindah akibat collision dari slot 3</p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-800">
              ...
              <ul className="list-disc list-inside space-y-0.5">
                <li>3 dari 5 key (10, 3, 17) memiliki hash = 3 → terjadi clustering di slot 3–5</li>
                <li>Load factor = 5/7 ≈ 0.71 — mendekati batas yang dianjurkan (0.7)</li>
                <li>Dengan chaining, semua key 10, 3, 17 akan tersimpan di linked list slot 3</li>
              </ul>
            </div>
  ...
  if (fase === 'loading') {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-gray-500">
        <svg className="animate-spin w-8 h-8 mb-4 text-teal-500" viewBox="0 0 24 24" fill="none">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
        </svg>
  ...
  if (fase === 'error') {
    return (
      <div className="py-10 text-center">
        ...
        <button onClick={() => generateSoal([])} className="px-4 py-2 bg-teal-600 text-white text-sm rounded-lg hover:bg-teal-700">
          Coba Lagi
        </button>
      </div>
    );
      ...
      <div className="text-[15px] text-gray-700">
        ...
        <div className="bg-gradient-to-r from-teal-600 to-teal-700 rounded-xl p-5 text-white mb-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-teal-200 text-sm font-medium">Nilai Rata-rata</p>
              <p className="text-4xl font-bold">{avgSkor}<span className="text-xl text-teal-300">/100</span></p>
            </div>
        ...
        <div className="flex items-center gap-2 mb-5">
          {soalList.map((sq, i) => {
            ...
            else cls += i === resultIdx ? 'bg-white border-teal-600 text-teal-600' : 'bg-gray-100 border-gray-300 text-gray-400';
            return (
        ...
        {curSoal && curFb && (
          <div className="border border-gray-200 rounded-xl overflow-hidden mb-4">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-teal-600 text-white flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{curSoal.id}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${curSoal.tipe === 'implementasi' ? 'bg-teal-50 text-teal-700 border-teal-200' : 'bg-indigo-50 text-indigo-700 border-indigo-200'}`}>
    ...
    <div className="text-[15px] text-gray-700">
      ...
      <div className="flex items-center gap-2 mb-5">
        {soalList.map((sq, i) => {
          ...
          let cls = 'border-2 ';
          if (f) cls += f.skor >= 70 ? 'bg-green-500 border-green-500 text-white' : 'bg-red-400 border-red-400 text-white';
          else if (isCur) cls += jawaban[sq.id]?.trim() ? 'bg-teal-600 border-teal-600 text-white' : 'bg-white border-teal-600 text-teal-600';
          else cls += jawaban[sq.id]?.trim() ? 'bg-teal-100 border-teal-400 text-teal-700' : 'bg-gray-100 border-gray-300 text-gray-400';
          return (
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-start gap-3">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 ${currentFeedback ? (currentFeedback.skor >= 70 ? 'bg-green-500 text-white' : 'bg-red-400 text-white') : (currentJawaban.trim() ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-500')}`}>
            {currentJawaban.trim() && !currentFeedback ? '✓' : soal.id}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${soal.tipe === 'implementasi' ? 'bg-teal-50 text-teal-700 border-teal-200' : 'bg-indigo-50 text-indigo-700 border-indigo-200'}`}>
...
function RingkasanContent() {
  return (
    <div className="text-[15px] text-gray-700">
      ...
      <div className="mb-5">
        <h2 className="text-xl font-bold text-gray-900">Ringkasan — ADT Set dan Map</h2>
        <p className="text-sm text-gray-400 mt-0.5">Set, Map, Hash Table, dan Collision Handling</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        ...
        <div className="border border-teal-100 rounded-xl overflow-hidden">
          <div className="bg-teal-600 px-4 py-2.5 flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-white text-teal-600 text-xs font-bold flex items-center justify-center shrink-0">1</span>
          ...
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[13px]">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                <div className="font-bold mb-1 text-gray-700">Operasi kunci</div>
...
setDifference, isSubset`}</code>
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 text-[12px]">
              ...
              <div className="bg-blue-50 border border-blue-100 text-blue-800 rounded-lg px-2 py-1.5 text-center">
                ...
                <div className="text-[11px] mt-0.5">add O(n) | isIn O(log n)</div>
              </div>
...
unset(k, M) → hapus`}</code>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-[12px]">
              ...
              <span className="text-gray-600 ml-1">associative array, symbol table, dictionary</span>
            </div>
          </div>
        </div>
        ...
        <div className="border border-orange-100 rounded-xl overflow-hidden">
          <div className="bg-orange-600 px-4 py-2.5 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">Hash Table & Collision</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[13px]">
              <div className="bg-orange-50 border border-orange-100 text-orange-800 rounded-lg px-3 py-2">
                ...
                <div className="text-[12px] opacity-90">Setiap slot = linked list. Collision → tambah ke list slot yang sama.</div>
              </div>
              <div className="bg-yellow-50 border border-yellow-100 text-yellow-800 rounded-lg px-3 py-2">
                ...
                <div className="text-[12px] opacity-90">Collision → cari slot kosong berikutnya. Linear: +1. Quadratic: +1,+4,+9.</div>
              </div>
            </div>
            <div className="mt-2 grid grid-cols-2 gap-2 text-[12px]">
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                <div className="font-bold text-gray-700 mb-0.5">Load Factor</div>
                <code className="font-mono text-gray-600">terisi / total ≤ 0.7</code>
              </div>
              <div className="bg-red-50 border border-red-100 rounded-lg px-3 py-2">
                <div className="font-bold text-red-700 mb-0.5">Hapus ≠ kosongkan slot</div>
                <div className="text-red-700 text-[11px]">Perlu tata ulang elemen collision-chain</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          <div className="bg-gray-800 px-4 py-2.5 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">Perbandingan Kinerja</span>
          </div>
          <div className="px-4 py-3">
            <div className="grid grid-cols-1 gap-2 text-[13px]">
              {[
                { impl: 'Array tak terurut', isIn: 'O(n)',       add: 'O(1)',       del: 'O(n)',       color: 'bg-red-50 border-red-100 text-red-800' },
                { impl: 'Array terurut',     isIn: 'O(log n)',   add: 'O(n)',       del: 'O(n)',       color: 'bg-yellow-50 border-yellow-100 text-yellow-800' },
                { impl: 'Hash table',        isIn: 'O(1) avg',   add: 'O(1) avg',  del: 'O(1) avg',  color: 'bg-green-50 border-green-100 text-green-800' },
              ].map((row) => (
                <div key={row.impl} className={`border rounded-lg px-3 py-2 flex items-center gap-3 ${row.color}`}>
                  <span className="font-bold text-[12px] w-36 shrink-0">{row.impl}</span>
                  <span className="font-mono text-[11px]">isIn: {row.isIn}</span>
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-xl p-4 mb-6">
        <p className="text-sm font-bold text-teal-800 mb-2">Axiomatic Semantics — Aturan Paling Penting</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[12px]">
          <div>
            <div className="text-[11px] font-bold text-teal-600 uppercase tracking-widest mb-1">Set</div>
...
isEmpty(add(v, S))  = false`}</pre>
          ...
          <div>
            <div className="text-[11px] font-bold text-indigo-600 uppercase tracking-widest mb-1">Map</div>
...
unset(k, new())      = new()`}</pre>
      ...
      <div className="border border-gray-200 rounded-xl p-4 bg-gray-50">
        ...
        <div className="grid grid-cols-1 gap-2 text-[13px]">
          {[
            { tip: 'Set vs Map',            detail: 'Set: elemen unik. Map: key unik, tiap key punya value.', color: 'bg-teal-50 border-teal-100 text-teal-800' },
            { tip: 'Remove di Set tak terurut', detail: 'Pindahkan elemen terakhir ke posisi yang dihapus → O(1) extra work', color: 'bg-blue-50 border-blue-100 text-blue-800' },
            { tip: 'set() di Map',           detail: 'Jika key sudah ada → update value. Jika belum → insert baru.', color: 'bg-indigo-50 border-indigo-100 text-indigo-800' },
            { tip: 'Hapus di Hash Table',    detail: 'Tidak bisa langsung kosongkan slot — harus tata ulang elemen chain.', color: 'bg-red-50 border-red-100 text-red-800' },
          ].map((item) => (
              ...
              return (
                <button
                  ...
                  className={`shrink-0 px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-t-lg transition-colors flex items-center gap-1.5 ${
                    isActive ? 'bg-teal-700 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                >
                  {tab}
                  {isDone && (
                ...
                {SECTIONS.map((s) => (
                  <button
                    ...
                    className={`w-full text-left text-[13px] px-4 py-2 transition-colors ${
                      ...
                      activeSection === s.id ? 'bg-teal-50 text-teal-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'
                    }`}
                  >
                    {s.title}
                  </button>
